...
// SetGraphemeCluster writes a grapheme cluster at the given point, relative to the top-left
// corner of the terminal. It checks if the position is valid and applies the given colors.
// Use this instead of SetRune when the string might contain combining characters (e.g., emoji with FE0F).
func (g *Gui) SetGraphemeCluster(x, y int, cluster string, fgColor, bgColor Attribute) error {
	if x < 0 || y < 0 || x >= g.maxX || y >= g.maxY {
		// swallowing error because it's not that big of a deal
		return nil
	}
	tcellSetCell(x, y, cluster, fgColor, bgColor, g.outputMode)
	return nil
}
...
func (g *Gui) drawTitle(v *View, fgColor, bgColor Attribute) error {
	...
	for gr.Next() {
		cluster := gr.Str()
		if err := g.SetGraphemeCluster(x, v.y0, cluster, fgColor, bgColor); err != nil {
			return err
		}
		x += gr.Width()
	}
	...
	for gr.Next() {
		...
		if charIndex >= currentTabStart && charIndex <= currentTabEnd {
			currentFgColor = v.SelFgColor
			if v != g.currentView {
				currentFgColor &= ^AttrBold
			}
		}
		if err := g.SetGraphemeCluster(x, v.y0, cluster, currentFgColor, currentBgColor); err != nil {
			return err
		}
		x += clusterWidth
		charIndex += len(cluster)
...
func (g *Gui) drawSubtitle(v *View, fgColor, bgColor Attribute) error {
	...
	x := start
	gr := uniseg.NewGraphemes(v.Subtitle)
	for gr.Next() {
		if x >= v.x1 {
			break
		}
		if err := g.SetGraphemeCluster(x, v.y0, gr.Str(), fgColor, bgColor); err != nil {
			return err
		}
		x += gr.Width()
	}
...
func (g *Gui) drawListFooter(v *View, fgColor, bgColor Attribute) error {
	...
	for gr.Next() {
		...
		if err := g.SetGraphemeCluster(x, v.y1, gr.Str(), fgColor, bgColor); err != nil {
			return err
		}
		x += gr.Width()
	}
